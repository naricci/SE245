...
namespace Assign2_ContactForm
{
    class FileIO
    {
            ...
            try
            {
                ...
                try
                {
                    ...
                    while (sr.Peek() != -1)
                    {
                        ...
                        // then add to listbox
                        string info = columns[0] + "," + columns[1] + "," + columns[3];
        ...
        //*******************************************************************************
        // Reads the Contacts file, line-by-line and gets data for each contact.
        //*******************************************************************************
        ...
        /* UPDATE FUNC */
        public static string UpdateFile(string myFileName, List<string> fname, List<string> lname, List<string> street1, List<string> street2,
            List<string> city, List<string> state, List<string> zipcode, List<string> email, List<string> hphone, List<string> wphone, List<string> cphone, List<string> cardworthy, List<string> relationship)
        { // creates an update file
            int NumofRecs = fname.Count - 1;
            int Index = 0;
            string myString = "";
            string strFeedback = "";
            // declares a Stream Writer
            StreamWriter sw;

            try
            { // deletes the old file and replaces it with the new
                ...
                for (int Ctr = 0; Ctr <= NumofRecs; Ctr++)
                {
                    ...
                    // all the strings
                    myString = fname[Ctr] + "," + lname[Ctr] + "," + street1[Ctr] + "," + street2[Ctr] + "," + city[Ctr] + "," + state[Ctr] + "," + zipcode[Ctr] + ","
                + email[Ctr] + "," + hphone[Ctr] + ", " + wphone[Ctr] + "," + cphone[Ctr] + "," + cardworthy[Ctr] + "," + relationship[Ctr];
